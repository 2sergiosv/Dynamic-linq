...
namespace DRR.Application.Common.Extensions
{
    ...
    {
        public static IQueryable<TEntity> OrderBy<TEntity>(this IQueryable<TEntity> source, IHasOrderBy orderByStrValues) where TEntity : class
        {
            if (string.IsNullOrEmpty(orderByStrValues.OrderByExpression))
                return source;
            ...
            var orderByValues = orderByStrValues.OrderByExpression.Trim().Split(',').Select(x => x.Trim()).ToList();
